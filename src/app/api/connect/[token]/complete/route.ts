...
import { connectedNote, scoreLine } from "@/lib/coach/score-page";
...
export async function POST(_req: Request, ctx: RouteContext<"/api/connect/[token]/complete">) {
  ...
  try {
    ...
    if (!read.loggedIn) return NextResponse.json({ ok: false, reason: "not_logged_in", notes: read.reason ?? read.extraction?.notes ?? "Still on the login page." });
    ...
    // The coach speaks: summary of what it saw, plan v1, and the long game. The marker is set first so that if this
    // invocation dies (timeout, lock, model error) the user's next text, whatever it says, still carries the follow-up.
    const note = connectedNote(x);
    await store.setPendingFollowup(link.user_id, note);
    let sent = false;
    try {
      sent = (await runEventTurn(link.user_id, { trigger: "inbound", intent: "credit_karma_connected", skills: ["onboarding", "plan_and_goals", "credit_coach"], notes: [note], instruction: connectedInstruction() })).sent;
    } catch (e) {
      console.error("connected turn failed; sending fallback", e);
      try {
        const user = await store.getUser(link.user_id);
        const text = x.score !== null ? `Connected 👽 Your Credit Karma score right now: ${scoreLine(x)}. Text me anything and I’ll send your summary and plan.` : `Connected 👽 I got in but couldn’t read a score on the page (${x.notes}). Open the score card once in Credit Karma, then text me "check my score".`;
        ...
        console.error("fallback text failed", e2);
      }
    }
    if (sent) await store.clearPendingFollowup(link.user_id);
    return NextResponse.json({ ok: true, score: x.score, score_model: x.score_model, bureau: x.bureau, as_of: x.as_of, followup: sent ? "sent" : "pending" });
  } catch (e) {
