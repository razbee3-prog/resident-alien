...
There is no card and no bureau contract yet. Behind `CREDIT_BROWSER_ENABLED=1` (plus `BROWSERBASE_API_KEY`, `BROWSERBASE_PROJECT_ID`), the coach can send a one-time link to `/connect/<token>` (15 minutes). On a laptop the page opens a Browserbase session in a live-view iframe; the user logs into Credit Karma themselves (nothing they type touches our servers), taps "I'm logged in", and `POST /api/connect/<token>/complete` navigates the same session to the Credit Health page (`/credit-health`, not the home page: for a logged-in user the home page bounces through Intuit's `/update?code=…` hand-off, which can sit on a spinner), screenshots it, and has Opus 5 read the score into a structured snapshot (`coach_credit_snapshots`). The route then fires a coach-initiated turn (`runEventTurn` with `connectedInstruction()`): summary of what it saw, plan v1, the long game, this week's action. Before that turn the user's profile gets a `pending_followup` marker; it is cleared once the text is sent, and if the invocation dies first, the user's next inbound message, whatever it says, carries the same note and instruction, so the follow-up is never lost. The session's cookies persist in a per-user Browserbase context (`coach_connections.context_id`, encrypted at rest there); the weekly check-in re-reads the score through that context and mentions it only when it is the first score or moved 10+ points. If the session has expired the weekly message carries a fresh link. Texting DISCONNECT deletes the context. On phones the live view takes taps but not the keyboard, so the page adds a text box that sends what the user types to the remote page over the session's own Chrome DevTools channel (`src/components/remote-keyboard.ts`): phone → Browserbase, never through our server. Browserbase's mobile OS emulation is Enterprise-only, so phones get a 390×844 viewport on Linux Chrome instead.

On a phone, tapping "I'm logged in" does not read from the page: it opens Messages with "I’m logged in 👽 read my score" drafted to Credit Alien (`smsDraftHref` in `src/lib/sms.ts`). Sending that text is the hand-off. The inbound turn sees an open link with a live session and reads *that* browser in place (`readThroughConnection` with `liveLink`), because its cookies only reach the saved context once the session ends; on success it stores the snapshot, marks the link completed, releases the session, and presents the plan in the same thread. If the page still shows the login, the coach asks them to finish and text again, without issuing a new link. Laptops keep the in-page read (`/complete`), with "Read it here instead" as the phone fallback.

"Check my score" from iMessage (router `task_signal=wants_score_check`, or any nudge about it while a link is open) reads through the saved context right inside the turn (`readThroughConnection` in `weekly.ts`, 60 s budget) and the reply quotes the fresh number; only when the saved session has expired does the coach send a new login link. The same helper feeds the weekly check-in's SCORE UPDATE line.

Phone path: the user texts a screenshot of their score card (Credit Karma, Experian, a bank app). `turn.ts` fetches the attachment, runs the same extractor, records a `screenshot` snapshot, and the coach confirms the score with model, bureau, and date. The weekly check-in may ask for a fresh screenshot when there is no browser connection and the score on file is stale.
